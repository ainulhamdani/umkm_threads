  ...
  return (
    <>
      ...
      <aside className="cart-drawer" role="dialog" aria-modal="true" aria-labelledby="cart-heading">
        <div className="cart-header"><h2 id="cart-heading">{ui.cart}</h2><button className="button button-text" type="button" onClick={onClose}>{ui.close}</button></div>
        <p className="muted">{shopName}</p>
        {lines.length === 0 ? <div className="empty-state">{ui.emptyCart}</div> : (
          <>
            <div className="cart-lines">
              {lines.map((line) => <div className="cart-line" key={line.product.id}><div><strong>{line.product.name}</strong><p className="muted">{line.quantity} × {formatIdr(line.product.priceIdr)}</p></div><div><strong>{formatIdr(line.product.priceIdr * line.quantity)}</strong><button className="button button-text" type="button" onClick={() => onRemove(line.product.id)}>Hapus</button></div></div>)}
            </div>
            ...
            <form className="form-card cart-form" onSubmit={checkout}>
              ...
              <button className="button button-primary" type="submit" disabled={submitting}>{submitting ? ui.loading : ui.orderWhatsapp}</button>
            </form>
          </>
        )}
      </aside>
